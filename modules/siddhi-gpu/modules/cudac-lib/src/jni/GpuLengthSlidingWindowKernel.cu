...
__global__ void ProcessEventsLengthSlidingWindow(
...
)
{
	...
	char * pResultsExpiredEvent = _pResultsBuffer + (_iSizeOfEvent * iEventIdx);
	char * pResultsInEvent = pResultsExpiredEvent + _iSizeOfEvent;
	...
	if(iEventIdx >= _iRemainingCount)
	{
		...
		if(iExpiredEventPos < 0)
		{
			...
			if(pWindowEvent->i_Type != GpuMetaAttribute::NONE) // if window event is filled
			{
				memcpy(pResultsExpiredEvent, pExpiredOutEventInWindowBuffer, _iSizeOfEvent);
//				GpuEvent * pExpiredEvent = (GpuEvent*) pResultsExpiredEvent;
//				pExpiredEvent->i_Type = GpuMetaAttribute::EXPIRED;
			}
			else
			{
				memset(pResultsExpiredEvent, 0, _iSizeOfEvent);
				GpuEvent * pExpiredEvent = (GpuEvent *)pResultsExpiredEvent;
				pExpiredEvent->i_Type = GpuMetaAttribute::NONE;
			}
		}
		else
		{
			...
			memcpy(pResultsExpiredEvent, pExpiredOutEventInInputBuffer, _iSizeOfEvent);
			GpuEvent * pExpiredEvent = (GpuEvent*) pResultsExpiredEvent;
			pExpiredEvent->i_Type = GpuMetaAttribute::EXPIRED;
		}
	}
	...
	memcpy(pResultsInEvent, pInEvent, _iSizeOfEvent);

}

__global__ void ProcessEventsLengthSlidingWindowFilter(
		char               * _pInputEventBuffer,     // original input events buffer
		GpuKernelMetaEvent * _pMetaEvent,            // Meta event for original input events
		int                  _iNumberOfEvents,       // Number of events in input buffer (matched + not matched)
...
__global__ void SetWindowState(
		char               * _pInputEventBuffer,     // original input events buffer
		int                  _iNumberOfEvents,       // Number of events in input buffer (matched + not matched)
		char               * _pEventWindowBuffer,    // Event window buffer
		int                  _iWindowLength,         // Length of current events window
...
)
{
	...
	if(pDestinationEvent->i_Type != GpuMetaAttribute::NONE) // there is an event in destination position
	{
	...
	pDestinationEvent->i_Type = GpuMetaAttribute::EXPIRED;
}
...
bool GpuLengthSlidingWindowFilterKernel::Initialize(GpuMetaEvent * _pMetaEvent, int _iInputEventBufferSize)
{
	...
	fprintf(fp_Log, "[GpuLengthSlidingWindowFilterKernel] OriginalInpuEventBufferIndex=%d\n", 0);
	fflush(fp_Log);
	p_OriginalEventBuffer = (GpuStreamEventBuffer*) p_Context->GetEventBuffer(0);

	// set input event buffer
	fprintf(fp_Log, "[GpuLengthSlidingWindowFilterKernel] InpuEventBufferIndex=%d\n", i_InputBufferIndex);
	fflush(fp_Log);
	p_InputEventBuffer = (GpuIntBuffer*) p_Context->GetEventBuffer(i_InputBufferIndex);
...
void GpuLengthSlidingWindowFilterKernel::Process(int _iNumEvents, bool _bLast)
{
	...
	ProcessEventsLengthSlidingWindowFilter<<<numBlocks, numThreads>>>(
			p_OriginalEventBuffer->GetDeviceEventBuffer(),
			p_OriginalEventBuffer->GetDeviceMetaEvent(),
			_iNumEvents,
			p_InputEventBuffer->GetDeviceEventBuffer(),
			p_WindowEventBuffer->GetDeviceEventBuffer(),
			i_WindowSize,
