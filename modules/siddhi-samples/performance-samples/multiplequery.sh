...
SIDDHI_ROOT=/home/prabodha/project/siddhi-git-dev
...
APP=org.wso2.siddhi.performance.JoinMultipleQueryPerformance
LOG=logs/JoinMultipleQueryPerformance/JoinMultipleQueryPerformance
#APP=org.wso2.siddhi.performance.ComplexFilterMultipleQueryPerformance
#LOG=logs/ComplexFilterMultipleQueryPerformance/ComplexFilterMultipleQueryPerformance
...
for q in 2 3 4; do
c=$((500000 * r))
echo "Executing --query-count ${q} --event-count ${c} --ringbuffer-size ${r} --threadpool-size ${t} --events-per-tblock ${b}"
${JVM} ${AGENTLIB} -classpath ${CLASSPATH} ${APP} --enable-async false --enable-gpu false --query-count ${q} --event-count ${c} --ringbuffer-size ${r} --threadpool-size ${t} --events-per-tblock ${b} &>${LOG}_cpu_st_${q}_${r}_${t}_${b}.log
${JVM} ${AGENTLIB} -classpath ${CLASSPATH} ${APP} --enable-async true --enable-gpu false --query-count ${q} --event-count ${c} --ringbuffer-size ${r} --threadpool-size ${t} --events-per-tblock ${b} &>${LOG}_cpu_mt_${q}_${r}_${t}_${b}.log
${JVM} ${AGENTLIB} -classpath ${CLASSPATH} ${APP} --enable-async true --enable-gpu true --query-count ${q} --event-count ${c} --ringbuffer-size ${r} --threadpool-size ${t} --events-per-tblock ${b} &>${LOG}_gpu_${q}_${r}_${t}_${b}.log
done
done
done
done
